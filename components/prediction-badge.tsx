...
export function PredictionBadge({ prediction }: { prediction?: Prediction }) {
  ...
  const confidencePct = Math.round((isReal ? prediction.real_confidence : prediction.fake_confidence));
  ...
  return (
    <Popover defaultOpen>
      ...
      <PopoverContent className="w-fit">
        <div className="flex flex-col gap-2"><span className="text-xs font-medium text-muted-foreground">Vote for this prediction</span>
          <PredictionVote /></div>
      </PopoverContent>
    </Popover>
  );
